 ...
 */
class EndpointParameterParser implements ParserInterface
{
    /** @var ParserInterface */
    ...
    {
        $rawParameter = $specification->getSchema($pointer);
        $error = $this->validateEndpointParameter($rawParameter);

        if ($error) {
            $parameter = new InvalidObject($error);
        } else {
            $parameter = $this->parseEndpointParameter($specification, $rawParameter, $pointer);
        }

        return $parameter;
    }

    private function validateEndpointParameter(array $rawParameter): string
    {
        $error = '';
